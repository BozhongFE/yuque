<template>
  <div class="home">
    <div
      class="artboards-container"
      @touchstart.prevent="handleTouchStart"
      @touchend="handleTouchEnd"
    >
      <div class="artboards-wrapper">
        <div class="artboards__bg">
          <div class="artboards-arc__bg">
            <div class="artboards-datas">
              <span class="desc-data__text">好孕率</span>
              <span class="detail-data__text">
                <span class="detail-data--number">{{ this.curData.haoyun }}</span>
                <span class="detail-data--percentage">%</span>
              </span>
              <span class="desc-data__text">黄体期</span>
              <div class="arrow-icon__bg"></div>
            </div>
          </div>
        </div>
        <ul class="cycle--list" :style="{transform: `rotate(${rotateDeg}deg)`}">
          <li class="cycle--item-wrapper" v-for="(item, index) in datas" :key="index">
            <div
              :class="['cycle--item', {'active': index === curIndex}]"
              :style="{transform: `rotate(${180 - (index + 5) * 60}deg)`}"
            >
              <h3 class="cycle--title">周期</h3>
              <span class="cycle--day">第{{ item.period_days }}天</span>
            </div>
          </li>
        </ul>
      </div>
...
.artboards-wrapper .artboards__bg {
  ...
  width: inherit;
  height: inherit;
  background: url(../../assets/artboards/pink-artboard.png) center bottom
    no-repeat;
  background-size: 100%;
}
